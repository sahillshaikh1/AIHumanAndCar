...
public class TrafficSystem : MonoBehaviour {
    ...
    void Awake()
    {
        ...
        for (int i = 0; i <3; i++)//cantrolling num of car in secne
        {

            GameObject ga = Instantiate(IaCars[Random.Range(1,IaCars.Length)], new Vector3(i * 4, 0, i * 4), Quaternion.identity);

            ga.GetComponent<TrafficCar>().atualWay = GameObject.FindGameObjectWithTag("Path");
            ga.GetComponent<TrafficCar>().path = GameObject.FindGameObjectWithTag("Path");
